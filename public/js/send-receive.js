...
async function loadReceiveUSD() {
    ...
    try {
        ...
        var acct = (res.data && res.data[0]) ? res.data[0] : await createUserAccount() || { account_number: 'MT' + Date.now().toString().slice(-8), balance: 0 };
        ...
        body.innerHTML = '<div>'
            ...
            + receiveRow('Bank Name', 'Monteriq Bank', true)
            + receiveRow('SWIFT / BIC', 'MONQUS33', true)
...
function downloadCryptoAddress(coin, address) {
    ...
    var text = 'MONTERIQ BANK — ' + coinLabel.toUpperCase() + ' WALLET\n'
        + '========================================\n'
    ...
    a.href = url; a.download = 'monteriq-' + coin + '-wallet.txt'; a.click();
...
function copyAccountDetails() {
    ...
    var name = ((currentUser.first_name || '') + ' ' + (currentUser.last_name || '')).trim();
    var text = 'MONTERIQ BANK — ACCOUNT DETAILS\n================================\n'
        + 'Date:             ' + new Date().toLocaleDateString() + '\n\n'
        + 'Account Holder:   ' + name + '\n'
        + 'Account Number:   ' + num.trim() + '\n'
        + 'Routing Number:   021000021\n'
        + 'Bank Name:        Monteriq Bank\n'
        + 'SWIFT / BIC:      MONQUS33\n'
        + 'Currency:         USD\n\n'
...
function downloadAccountDetails() {
    ...
    var url  = URL.createObjectURL(blob);
    var a    = document.createElement('a');
    a.href = url; a.download = 'monteriq-account-' + num.trim() + '.txt'; a.click();
    URL.revokeObjectURL(url);
